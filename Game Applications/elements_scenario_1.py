...
def draw_frame_scenario_1(image_scenario_1, window_width, window_heigth, window, Image_waste, state_elements):
    ...
    if state_elements["casca_banana"] == 0:
        ...
        resized_image = pygame.transform.scale(Image_waste['casca_banana'], (100, 100))  # Redimensiona a imagem se necessário

        x = int(rect_text.width/2) + int(text_rect_text.get_width()/2) + elements_already_deposited_correct*110 + 15
    ...
    if state_elements["jornal"] == 0:
        ...
        resized_image = pygame.transform.scale(Image_waste['jornal'], (100, 100))  # Redimensiona a imagem se necessário
    ...
    if state_elements["lata_refrigerante"] == 0:
        ...
        resized_image = pygame.transform.scale(Image_waste['lata_refrigerante'], (100, 100))  # Redimensiona a imagem se necessário
    ...
    if (state_elements["casca_banana"] + state_elements["jornal"] + state_elements["lata_refrigerante"])%2 == 1:
        font = pygame.font.Font('Fonts/PoetsenOne-Regular.ttf', 45)
        ...
        font = pygame.font.Font('Fonts/PoetsenOne-Regular.ttf', 35)
        rect_text = pygame.Rect(window_width*0.5- larg/2, window_heigth*0.5 -(alt1+alt2)/2 + alt1-1, larg, alt2)
        pygame.draw.rect(window, (203,238,203), rect_text)

        linhas = wrap_text(string, font, rect_text.width - 20)

        y_offset = rect_text.y + 10  # Deslocamento inicial dentro do retângulo
        for linha in linhas:
            if y_offset + 40 > rect_text.y + rect_text.width:  # Verifica se a linha cabe dentro do retângulo
                break
            texto = font.render(linha, True, (0, 100, 0))
            window.blit(texto, (rect_text.x + 10, y_offset))
